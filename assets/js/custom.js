...
function d3drawAxis(target, chartSize) {
	...
	gaxisX.attr(
		"transform",
		"translate(" + linePadding.left + "," + (chartSize.h - linePadding.bottom) + ")"
	);
	gaxisY.attr(
		"transform",
		"translate(" + linePadding.left + "," + linePadding.top + ")" + ""
	...
	gaxisX
		...
		.each(function (d, i) {
			...
			// if (i == 0 || i + 1 == valArrX.length) {
			// 	this.remove();
			// }
			if (window.innerWidth < 1280 && i % 2 == 1 && i !== 0 && i + 1 !== valArrX.length) {
				this.remove();
			}
		});
...
function handleMouseOver(d, i) {
	var colorArr = ["#66a222", "#c62f70", "#00aedb", "#a200ff", "#f47835", "#d41243"];
	var colorIndex;
	...
	d3.select(this)
		.classed("aniDots", true)
		.attr("stroke", colorArr[colorIndex]);
}
...
function fillCircle(target) {
	...
	target
		...
		.attr("cx", function (d, i) {
			// ????
			circleXpos = parseInt((i * lineRealWidth) / (dataSet.length - 1) + linePadding.left);

			return parseInt((i * lineRealWidth) / (dataSet.length - 1) + linePadding.left);
		})
		// circle 's y position
		...
		.attr("r", function () {
			var dSize;
			if (window.innerWidth > 768) { dSize = 10 }
			else {
				dSize = 6
			}
			return dSize;
...
var snowPeopleImg = document.getElementsByClassName("snow-people")[0].children[0];
...
function verifyRangeSliderOrient(e) {
	...
	$("#rangeSlider").slider({
		...
		change: function (event, ui) {
			...
			if (!event.detail || event.detail == 1 && preventSliderVal !== ui.value) {
...
function showCorrespondArea(target, index) {
	...
	var snowBallBubbleInfo = [
		"在<span class='sp'>2020</span>年期間<br>投資 S&P500 指數累積報酬率高達：<span class='sp'>+16.26%</span>，<br>你滾出了一顆小雪球，但其實還有更大的 切換時間點找出大雪球吧",
		"在<span class='sp'>2017</span>年至<span class='sp'>2020</span>年間<br>投資 S&P500 指數累積報酬率高達：<span class='sp'>+40.49%</span>，<br>你滾出了一顆小雪球，但其實還有更大的 切換時間點找出大雪球吧",
		"在<span class='sp'>2015</span>年至<span class='sp'>2020</span>年間<br>投資 S&P500 指數累積報酬率高達：<span class='sp'>+83.77%</span>，<br>你滾出了一顆中雪球，但其實還有更大的 切換時間點找出大雪球吧",
		"在<span class='sp'>2010</span>年至<span class='sp'>2020</span>年間<br>投資 S&P500 指數累積報酬率高達：<span class='sp'>+198.66%</span>，<br>恭喜你！滾出了一顆大雪球！<br>掌握長期投資心法，你也有機會滾出大雪球！"]
	var snowBallBubbleInfoShirnk = [
		"S&P500指數報酬率為:<span class='sp'>+16.26%</span><br>你滾出了一顆小雪球",
		"S&P500指數報酬率為:<span class='sp'>+40.49%</span><br>你滾出了一顆小雪球",
...
function snowBallWrapAniStagger(i) {
	...
	if (window.innerWidth <= 768) {
		snowBallSize = snowBallSizeArr[i] * .5;
		snowWrapLeftVal = snowBallSize * 1 + 20;
		walkingLapse = 4;
	}

	gsap.fromTo("#snowball-wrap",
		{
	...
	gsap.fromTo("#snow-ball", { width: (snowBallSize * .7) + 'px' }, { duration: walkingLapse, width: snowBallSize + "px" });
}
...
function keyVisualPopup() {
	...
	gsap.fromTo(".ani-deco", { opacity: 0, scale: (.5) }, { duration: .5, opacity: 1, scale: (1) });
}



function snowComplete() {
	snowPeopleImg.classList.remove('active');
	var snowInnerBubblePosXRef = snowballWrap.getBoundingClientRect();
	var snowInnerBubblePosX = null;
	window.innerWidth > 768 ? snowInnerBubblePosX = snowInnerBubblePosXRef.x - 380 - 80 : snowInnerBubblePosX = "unset"
	snowInnerBubble.style.left = snowInnerBubblePosX + "px";
